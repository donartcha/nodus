...
const getActivityText = (tool: NodusSayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: NodusMessage) => boolean,
...
const findCurrentApiReq = (messages: NodusMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: NodusMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = nodusMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, nodusMessages)
	}, [message.ts, nodusMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(nodusMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(nodusMessages, currentApiReq.index + 1, nodusMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return nodusMessages.some((msg, idx) => {
