...
export function validateModelId(
	...
	nodusModels?: Record<string, ModelInfo>,
): string | undefined {
	if (apiConfiguration) {
		const { apiProvider, openRouterModelId, NodusModelId } = getModeSpecificFields(apiConfiguration, currentMode)
		switch (apiProvider) {
			case "openrouter":
				...
				break
			case "Nodus":
				const NodusResolvedModelId = NodusModelId || openRouterDefaultModelId
				if (!NodusResolvedModelId) {
					return "You must provide a model ID."
				}
				if (nodusModels && !Object.keys(nodusModels).includes(NodusResolvedModelId)) {
					return "The model ID you provided is not available. Please choose a different model."
				}
				break
		}
