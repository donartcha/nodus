...
import {
	NodusAskUseSubagents,
	NodusSaySubagentStatus,
	NodusSubagentUsageInfo,
	SubagentStatusItem,
...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	readonly name = NodusDefaultTool.USE_SUBAGENTS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const partialMessage = JSON.stringify({ prompts } satisfies NodusAskUseSubagents)
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const approvalPayload: NodusAskUseSubagents = { prompts }
		...
		if (didAutoApprove) {
			...
			showNotificationForApproval(
				prompts.length === 1
					? `Nodus wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
					: `Nodus wants to use ${prompts.length} subagents`,
		...
		const emitStatus = async (status: NodusSaySubagentStatus["status"], partial: boolean) => {
			...
			const payload: NodusSaySubagentStatus = {
				status,
		...
		const queueStatusUpdate = (status: NodusSaySubagentStatus["status"], partial: boolean): Promise<void> => {
		...
		const subagentUsagePayload: NodusSubagentUsageInfo = {
			source: "subagents",
