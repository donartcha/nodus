...
import { NodusEndpoint, NodusEnv } from "@/config"
...
describe("Controller Marketplace Filtering", () => {
	...
	// Initialize NodusEndpoint before tests run (required for NodusEnv.config() to work)
	before(async () => {
		if (!NodusEndpoint.isInitialized()) {
			await NodusEndpoint.initialize("/test/extension")
		}
	})
	...
	describe("API interaction", () => {
		it("should call marketplace API with correct parameters", async () => {
			...
			callArgs[0].should.equal(`${NodusEnv.config().mcpBaseUrl}/marketplace`)
		})
