import type { UsageTransaction as NodusAccountUsageTransaction, PaymentTransaction } from "@shared/NodusAccount"
import { isNodusInternalTester } from "@shared/internal/account"
...
import { type NodusUser, handleSignOut } from "@/context/NodusAuthContext"
...
import { convertProtoUsageTransactions, getNodusUris, getMainRole } from "./helpers"
...
type NodusAccountViewProps = {
	NodusUser: NodusUser
	userOrganizations: UserOrganization[] | null
	activeOrganization: UserOrganization | null
	NodusEnv: "Production" | "Staging" | "Local"
}

type CachedData = {
	balance: number | null
	usageData: NodusAccountUsageTransaction[]
	paymentsData: PaymentTransaction[]
	lastFetchTime: number
}

const NodusEnvOptions = ["Production", "Staging", "Local"] as const

const AccountView = ({ onDone, NodusUser, organizations, activeOrganization }: AccountViewProps) => {
			...
			<div className="grow flex flex-col px-5 overflow-y-auto">
				{NodusUser?.uid ? (
					<NodusAccountView
						...
						NodusEnv={environment === "local" ? "Local" : environment === "staging" ? "Staging" : "Production"}
						NodusUser={NodusUser}
						key={NodusUser.uid}
...
export const NodusAccountView = ({ NodusUser, userOrganizations, activeOrganization, NodusEnv }: NodusAccountViewProps) => {
	const { email, displayName, appBaseUrl, uid } = NodusUser
	...
	const [usageData, setUsageData] = useState<NodusAccountUsageTransaction[]>([])
	...
	const isNodusTester = useMemo(() => (email ? isNodusInternalTester(email) : false), [email])
	...
	const NodusUrl = appBaseUrl || "https://app.nodus.bot"
			...
			<div className="flex flex-col h-full">
				...
				<div className="w-full flex gap-2 flex-col min-[225px]:flex-row">
					<div className="w-full min-[225px]:w-1/2">
						<VSCodeButtonLink appearance="primary" className="w-full" href={getNodusUris(NodusUrl, "dashboard").href}>
							Dashboard
						</VSCodeButtonLink>
					</div>
				...
				<CreditBalance
					balance={balance}
					creditUrl={getNodusUris(NodusUrl, "credits", dropdownValue === uid ? "account" : "organization")}
				...
				{isNodusTester && environment !== "selfHosted" && (
					<div className="w-full gap-1 items-end">
						<VSCodeDivider className="w-full my-3" />
						<div className="text-sm font-semibold">Nodus Environment</div>
						<VSCodeDropdown
							className="w-full mt-1"
							currentValue={NodusEnv}
							onChange={async (e) => {
