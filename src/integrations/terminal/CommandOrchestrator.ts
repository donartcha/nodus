...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const nodusMessages = callbacks.getnodusMessages()
		const lastCommandIndex = findLastIndex(nodusMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateNodusMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using NodusTempManager for proper cleanup
		largeOutputLogPath = NodusTempManager.createTempFilePath("large-output")
