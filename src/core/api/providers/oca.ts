...
export class OcaHandler implements ApiHandler {
	...
	async *createMessageChatApi(systemPrompt: string, messages: NodusStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
		...
		const chatCompletionsParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
			...
			...(this.options.taskId && {
				litellm_session_id: `Nodus-${this.options.taskId}`,
	...
	async *createMessageResponsesApi(systemPrompt: string, messages: NodusStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
	...
	async *createMessageMessagesApi(systemPrompt: string, messages: NodusStorageMessage[], tools?: OpenAITool[]): ApiStream {
