...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	/**
	 * Handle adding the thread conversation to Nodus's main chat
	 */
...
Please continue helping the user with their question about this code.`
	...
	/**
	 * Close all tabs that use the Nodus-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with Nodus-diff scheme (opened during comment reveal)
