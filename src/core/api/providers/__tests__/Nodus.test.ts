...
describe("NodusHandler", () => {
	...
	const createHandler = (options: ConstructorParameters<typeof NodusHandler>[0]) => {
		sinon.stub(NodusAccountService, "getInstance").returns({} as any)
		sinon.stub(AuthService, "getInstance").returns({} as any)
		return new NodusHandler(options)
	}
