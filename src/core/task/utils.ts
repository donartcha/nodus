...
import { NodusApiReqCancelReason, NodusApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const nodusMessages = params.messageStateHandler.getnodusMessages()
	const currentApiReqInfo: NodusApiReqInfo = JSON.parse(nodusMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateNodusMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
