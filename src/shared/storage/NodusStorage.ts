...
export interface NodusStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: NodusStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and NodusSyncStorage both satisfy this interface.
 */
export interface NodusMemento {
 ...
 */
export abstract class NodusStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "NodusStorage"
	/**
 ...
 * Unlike NodusStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: NodusStorageChangeEvent) => void

export abstract class NodusSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of NodusStorage using a Map.
 */
export class NodusInMemoryStorage extends NodusStorage {
	/**
