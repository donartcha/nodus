...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from nodusMessages?
		const nodusMessages = currentTask.messageStateHandler.getnodusMessages()
		const lastProgressMessage = nodusMessages
			.slice()
			.reverse()
