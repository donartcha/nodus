...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = NodusDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						nodusMessages: config.messageState.getnodusMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(NodusDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getnodusMessages(),
				config.api,
