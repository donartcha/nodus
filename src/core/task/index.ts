...
import { NodusIgnoreController } from "@core/ignore/NodusIgnoreController"
...
import { NodusApiReqCancelReason, NodusApiReqInfo, NodusAsk, NodusMessage, NodusSay } from "@shared/ExtensionMessage"
...
import { ensureLocalNodusDirExists } from "../context/instructions/user-instructions/rule-helpers"
...
export type ToolResponse = NodusToolResponseContent
...
export class Task {
	...
	private NodusIgnoreController: NodusIgnoreController
	...
	constructor(params: TaskParams) {
		...
		this.NodusIgnoreController = new NodusIgnoreController(cwd)
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				const nodusMessages = this.messageStateHandler.getnodusMessages()
				const lastApiReqStartedIndex = findLastIndex(nodusMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: NodusApiReqInfo = JSON.parse(nodusMessages[lastApiReqStartedIndex].text || "{}")
						...
						await this.messageStateHandler.updateNodusMessage(lastApiReqStartedIndex, {
		...
		const commandExecutorCallbacks: CommandExecutorCallbacks = {
			...
			ask: async (type: string, text?: string, partial?: boolean) => {
				const result = await this.ask(type as NodusAsk, text, partial)
				return {
			...
			resolvePendingAsk: (response) => {
				void this.handleWebviewAskResponse(response as NodusAskResponse)
			},
			updateBackgroundCommandState: (isRunning: boolean) =>
				this.controller.updateBackgroundCommandState(isRunning, this.taskId),
			updateNodusMessage: async (index: number, updates: { commandCompleted?: boolean; text?: string }) => {
				await this.messageStateHandler.updateNodusMessage(index, updates)
			},
			getnodusMessages: () => this.messageStateHandler.getnodusMessages() as Array<{ ask?: string; say?: string }>,
			addToUserMessageContent: (content: { type: string; text: string }) => {
				// Cast to NodusTextContentBlock which is compatible with NodusContent
				this.taskState.userMessageContent.push({ type: "text", text: content.text } as NodusTextContentBlock)
			},
		}
		...
		// before streaming begins (in recursivelyMakeNodusRequests) so the cadence is always
		...
		this.presentationScheduler = new TaskPresentationScheduler({
			...
			getDelayMs: (priority) => {
				if (!this.remoteWorkspaceDetectionSettled) {
					// This should never fire in production because recursivelyMakeNodusRequests
	...
	async ask(
		...
		if (this.taskState.abort && type !== "resume_task" && type !== "resume_completed_task") {
			throw new Error("Nodus instance aborted")
		}
		let askTs: number
		if (partial !== undefined) {
			const nodusMessages = this.messageStateHandler.getnodusMessages()
			const lastMessage = nodusMessages.at(-1)
			const lastMessageIndex = nodusMessages.length - 1
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.messageStateHandler.updateNodusMessage(lastMessageIndex, {
						text,
						partial,
					})
					// todo be more efficient about saving and posting only new data or one whole message at a time so ignore partial for saves, and only post parts of partial message instead of whole array in new listener
					// await this.savenodusMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertNodusMessageToProto(lastMessage)
				...
				await this.messageStateHandler.addTonodusMessages({
					ts: askTs,
					type: "ask",
					ask: type,
					text,
			...
			// const lastMessage = this.nodusMessages.at(-1)
			this.taskState.askResponse = undefined
			this.taskState.askResponseText = undefined
			this.taskState.askResponseImages = undefined
			this.taskState.askResponseFiles = undefined
	...
	async handleWebviewAskResponse(askResponse: NodusAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(
		...
		const modelInfo: NodusMessageModelInfo = {
		...
		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getnodusMessages().at(-1)
			const isUpdatingPreviousPartial =
				lastMessage && lastMessage.partial && lastMessage.type === "say" && lastMessage.say === type
			if (partial) {
				if (isUpdatingPreviousPartial) {
					// existing partial message, so update it
					const lastIndex = this.messageStateHandler.getnodusMessages().length - 1
					await this.messageStateHandler.updateNodusMessage(lastIndex, {
						text,
						images,
						files,
						partial,
			...
			if (isUpdatingPreviousPartial) {
				...
				const lastIndex = this.messageStateHandler.getnodusMessages().length - 1
				// updateNodusMessage emits the change event and saves to disk
				await this.messageStateHandler.updateNodusMessage(lastIndex, {
					text,
					images,
					files,
	...
	async sayAndCreateMissingParamError(toolName: NodusDefaultTool, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Nodus tried to use ${toolName}${
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: NodusAsk | NodusSay) {
		const nodusMessages = this.messageStateHandler.getnodusMessages()
		const lastMessage = nodusMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setnodusMessages(nodusMessages.slice(0, -1))
			await this.messageStateHandler.savenodusMessagesAndUpdateHistory()
		}
	}
	 ...
	 */
	private calculatePreCompactDeletedRange(apiConversationHistory: NodusStorageMessage[]): [number, number] {
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.NodusIgnoreController.initialize()
		} catch (error) {
			Logger.error("Failed to initialize NodusIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and nodusMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the nodusMessages might not be empty, so we need to set it to [] when we create a new Nodus client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setnodusMessages([])
		...
		const imageBlocks: NodusImageContentBlock[] = formatResponse.imageBlocks(images)

		const userContent: NodusUserContent[] = [
			{
				type: "text",
				text: `<task>\n${task}\n</task>`,
			},
	...
	public async resumeTaskFromHistory() {
		...
		const savednodusMessages = await getSavednodusMessages(this.taskId)
		...
		if (lastRelevantMessageIndex !== -1) {
			savednodusMessages.splice(lastRelevantMessageIndex + 1)
		}

		// since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
		const lastApiReqStartedIndex = findLastIndex(savednodusMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savednodusMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: NodusApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savednodusMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwritenodusMessages(savednodusMessages)
		this.messageStateHandler.setnodusMessages(await getSavednodusMessages(this.taskId))

		// Now present the Nodus messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastNodusMessage = this.messageStateHandler
			.getnodusMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: NodusAsk
		if (lastNodusMessage?.ask === "completion_result") {
		...
		const newUserContent: NodusContent[] = []
			...
			const taskResumeResult = await executeHook({
				...
				hookInput: {
					taskResume: {
						...
						previousState: {
							lastMessageTs: lastNodusMessage?.ts?.toString() || "",
							messageCount: nodusMessages.length.toString(),
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with Nodus messages
		...
		let modifiedOldUserContent: NodusContent[] // either the last message if its user message, or the user message before the last (assistant) message
		let modifiedApiConversationHistory: NodusStorageMessage[] // need to remove the last user message to replace with new modified user message
		if (existingApiConversationHistory.length > 0) {
			...
			if (lastMessage.role === "assistant") {
				...
				const existingUserContent: NodusContent[] = Array.isArray(lastMessage.content)
					? lastMessage.content
		...
		const agoText = (() => {
			const timestamp = lastNodusMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastNodusMessage?.ts && Date.now() - lastNodusMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: NodusContent[]): Promise<void> {
		let nextUserContent = userContent
		let includeFileDetails = true
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeNodusRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that Nodus will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			...
			// this.say(
			// 	"tool",
			// 	"Nodus responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
			// )
			nextUserContent = [
				{
					type: "text",
	...
	async abortTask() {
		try {
			...
			this.NodusIgnoreController.dispose()
	...
	): Promise<[boolean, NodusToolResponseContent]> {
	...
	public async cancelHookExecution(): Promise<boolean> {
		...
		try {
			...
			const nodusMessages = this.messageStateHandler.getnodusMessages()
			const hookMessageIndex = nodusMessages.findIndex((m) => m.ts === messageTs)
			if (hookMessageIndex !== -1) {
				...
				await this.messageStateHandler.updateNodusMessage(hookMessageIndex, {
		...
		if (hooksEnabled) {
			try {
				...
				await executePreCompactHookWithCleanup({
					...
					nodusMessages: this.messageStateHandler.getnodusMessages(),
					messageStateHandler: this.messageStateHandler,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const { globalToggles, localToggles } = await refreshNodusRulesToggles(this.controller, this.cwd)
		...
		const globalRules = await getGlobalNodusRules(globalNodusRulesFilePath, globalToggles, { evaluationContext })
		let globalNodusRulesFileInstructions = globalRules.instructions
		...
		if (lazyTeammateModeEnabled) {
			...
			globalNodusRulesFileInstructions = globalNodusRulesFileInstructions
				? `${globalNodusRulesFileInstructions}\n\n${LAZY_TEAMMATE_RULES}`
				: LAZY_TEAMMATE_RULES
		}

		const localRules = await getLocalNodusRules(this.cwd, localToggles, { evaluationContext })
		const localNodusRulesFileInstructions = localRules.instructions
		...
		const NodusIgnoreContent = this.NodusIgnoreController.NodusIgnoreContent
		let NodusIgnoreInstructions: string | undefined
		if (NodusIgnoreContent) {
			NodusIgnoreInstructions = formatResponse.NodusIgnoreInstructions(NodusIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			globalNodusRulesFileInstructions,
			localNodusRulesFileInstructions,
			localCursorRulesFileInstructions,
			localCursorRulesDirInstructions,
			localWindsurfRulesFileInstructions,
			localAgentsRulesFileInstructions,
			NodusIgnoreInstructions,
			...
			nodusWebToolsEnabled:
				this.stateManager.getGlobalSettingsKey("nodusWebToolsEnabled") && featureFlagsService.getWebtoolsEnabled(),
		...
		const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(
			...
			this.messageStateHandler.getnodusMessages(),
			this.api,
			this.taskState.conversationHistoryDeletedRange,
			previousApiReqIndex,
			await ensureTaskDirectoryExists(this.taskId),
		...
		try {
			...
			const NodusError = ErrorService.get().toNodusError(error, model.id, providerId)

			// Capture provider failure telemetry using NodusError
			ErrorService.get().logMessage(NodusError.message)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						NodusError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
						this.taskState.didAutomaticallyRetryFailedApiRequest = false
					}
				}

				const streamingFailedMessage = NodusError.serialize()
				...
				if (lastApiReqStartedIndex !== -1) {
					...
					await this.messageStateHandler.updateNodusMessage(lastApiReqStartedIndex, {
						...
						} satisfies NodusApiReqInfo),
					})
					// this.ask will trigger postStateToWebview, so this change should be picked up.
				}

				const isAuthError = NodusError.isErrorType(NodusErrorType.Auth)
				const isSpendLimitError = NodusError.isErrorType(NodusErrorType.SpendLimit)
				const quotaExceeded = NodusError.isErrorType(NodusErrorType.QuotaExceeded)

				// Check if this is a Nodus provider insufficient credits error - don't auto-retry these
				const isNodusProviderInsufficientCredits = (() => {
					if (providerId !== "Nodus") {
						return false
					}
					try {
				...
				let response: NodusAskResponse
				// Skip auto-retry for Nodus provider insufficient credits, auth errors, or spend limit errors
				const shouldRetry =
					!isNodusProviderInsufficientCredits &&
					!isAuthError &&
					!isSpendLimitError &&
					!quotaExceeded &&
				...
				if (shouldRetry) {
					...
					if (autoRetryApiReqIndex !== -1) {
						const nodusMessages = this.messageStateHandler.getnodusMessages()
						const currentApiReqInfo: NodusApiReqInfo = JSON.parse(nodusMessages[autoRetryApiReqIndex].text || "{}")
						delete currentApiReqInfo.streamingFailedMessage
						await this.messageStateHandler.updateNodusMessage(autoRetryApiReqIndex, {
							text: JSON.stringify(currentApiReqInfo),
						})
					}
					...
					const showRetry = !isNodusProviderInsufficientCredits && !isAuthError && !isSpendLimitError && !quotaExceeded
					if (showRetry) {
						await this.say(
							"error_retry",
							JSON.stringify({
				...
				if (manualRetryApiReqIndex !== -1) {
					const nodusMessages = this.messageStateHandler.getnodusMessages()
					const currentApiReqInfo: NodusApiReqInfo = JSON.parse(nodusMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateNodusMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async recursivelyMakeNodusRequests(userContent: NodusContent[], includeFileDetails = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Nodus is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images, files } = await this.ask(
				"mistake_limit_reached",
				this.api.getModel().id.includes("claude")
					? `This may indicate a failure in Nodus's thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
					: "Nodus uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4.5 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				...
				const feedbackUserContent: NodusUserContent[] = []
		...
		const previousApiReqIndex = findLastIndex(this.messageStateHandler.getnodusMessages(), (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.messageStateHandler.getnodusMessages().filter((m) => m.say === "api_req_started").length === 0
		...
		if (
			...
			try {
				...
				this.taskState.checkpointManagerErrorMessage = errorMessage // will be displayed right away since we savenodusMessages next which posts state to webview
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				commitPromise
					?.then(async (commitHash) => {
						if (commitHash) {
							await this.messageStateHandler.updateNodusMessage(lastCheckpointMessageIndex, {
								lastCheckpointHash: commitHash,
							})
							// savenodusMessagesAndUpdateHistory will be called later after API response,
		...
		if (shouldCompact) {
			...
			;[parsedUserContent, environmentDetails, NodusrulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getnodusMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateNodusMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies NodusApiReqInfo),
		})
		...
		try {
			...
			*/
			...
			const updateApiReqMsgFromMetrics = async (
				cancelReason?: NodusApiReqCancelReason,
			...
			const finalizeApiReqMsg = async (cancelReason?: NodusApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const abortStream = async (cancelReason: NodusApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const finalizePendingReasoningMessage = async (thinking: string): Promise<boolean> => {
				...
				await this.messageStateHandler.updateNodusMessage(pendingReasoningIndex, {
					text: thinking,
					partial: false,
				})
				const completedReasoning = this.messageStateHandler.getnodusMessages()[pendingReasoningIndex]
				if (completedReasoning) {
					await sendPartialMessageEvent(convertNodusMessageToProto(completedReasoning))
				}
				return true
			}
			...
			try {
				...
				while (true) {
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of Nodus)
				...
				await streamCoordinator?.stop()
				// abandoned happens when extension is no longer waiting for the Nodus instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					const NodusError = ErrorService.get().toNodusError(error, this.api.getModel().id)
					const errorMessage = NodusError.serialize()
					const isStreamingSpendLimitError = NodusError.isErrorType(NodusErrorType.SpendLimit)
			...
			// OpenRouter/Nodus may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantHasContent) {
				...
				const assistantContent: Array<NodusAssistantContent> = [
				...
				if (hasAssistantText) {
					assistantContent.push({
						...
						// reasoning_details only exists for Nodus/openrouter providers
			...
			if (assistantHasContent) {
				...
				const recDidEndLoop = await this.recursivelyMakeNodusRequests(this.taskState.userMessageContent)
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Nodus cannot process. Retrying the request may help resolve this issue."
	...
	async loadContext(
	...
	): Promise<[NodusContent[], string, boolean]> {
		let needsNodusrulesFileCheck = false
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			...
			const { processedText, needsNodusrulesFileCheck: needsCheck } = await parseSlashCommands(
				parsedText,
			...
			if (needsCheck) {
				needsNodusrulesFileCheck = true
			}

			return processedText
		}

		const processTextContent = async (block: NodusTextContentBlock): Promise<NodusTextContentBlock> => {
		...
		const processContentBlock = async (block: NodusContent): Promise<NodusContent> => {
			if (block.type === "text") {
		...
		// Check NodusrulesData if needed
		const NodusrulesError = needsNodusrulesFileCheck
			? await ensureLocalNodusDirExists(this.cwd, GlobalFileNames.NodusRules)
			: false
		...
		return [processedUserContent, environmentDetails, NodusrulesError]
	}
	...
	protected async processNativeToolCalls(assistantTextOnly: string, toolBlocks: ToolUse[]) {
		...
		// IMPORTANT: Finalize any partial text NodusMessage before we skip over it.
		//
		...
		//
		// Without this fix, the partial text NodusMessage remains with partial=true. In the UI
	...
	async getEnvironmentDetails(includeFileDetails = false) {
		...
		// It could be useful for Nodus to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through NodusIgnoreController
		const allowedVisibleFiles = this.NodusIgnoreController
		...
		// Filter paths through NodusIgnoreController
		const allowedOpenTabs = this.NodusIgnoreController
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.NodusIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: NodusMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const nodusMessages = this.messageStateHandler.getnodusMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(nodusMessages.slice(1)))
