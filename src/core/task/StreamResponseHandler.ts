...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: NodusAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to NodusAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): NodusAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: NodusAssistantToolUseBlock["reasoning_details"]): NodusAssistantToolUseBlock[] {
		const results: NodusAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(Nodus_MCP_TOOL_IDENTIFIER)) {
				...
				results.push({
					type: "tool_use",
					name: pending.name as NodusDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): NodusAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): NodusAssistantRedactedThinkingBlock[] {
