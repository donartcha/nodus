...
import type { NodusSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified NodusSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: NodusSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: NodusSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = NodusDefaultTool.GENERATE_EXPLANATION
