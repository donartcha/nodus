...
import { NodusDefaultTool, getToolUseNames } from "@/shared/tools"
...
You are a code reviewer.`
		...
		assert.deepEqual(parsed.tools, [NodusDefaultTool.FILE_READ, NodusDefaultTool.LIST_FILES, NodusDefaultTool.SEARCH])
		assert.equal(parsed.systemPrompt, "You are a code reviewer.")
	})

	it("supports raw Nodus tool ids in tools", () => {
		const content = `---
name: cli-agent
...
Prompt body`
		...
		const parsed = parseAgentConfigFromYaml(content)
		assert.deepEqual(parsed.tools, [NodusDefaultTool.FILE_READ, NodusDefaultTool.LIST_FILES])
	})
...
Reviewer prompt`,
		...
		assert.deepEqual(localAgent?.tools, [NodusDefaultTool.FILE_READ])
		assert.equal(localAgent?.systemPrompt, "Prompt body")
		assert.equal(reviewer?.name, "reviewer")
		assert.deepEqual(reviewer?.tools, [NodusDefaultTool.LIST_FILES])
