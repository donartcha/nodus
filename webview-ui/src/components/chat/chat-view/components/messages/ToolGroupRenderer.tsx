import { NodusMessage, NodusSayTool } from "@shared/ExtensionMessage"
import { StringRequest } from "@shared/proto/nodus/common"
...
const getCurrentActivities = (allMessages: NodusMessage[]): NodusMessage[] => {
 ...
 */
export function buildToolsWithReasoning(messages: NodusMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): NodusSayTool {
	try {
		return JSON.parse(text || "{}") as NodusSayTool
	} catch {
		return {} as NodusSayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: NodusSayTool) {
 ...
 */
export function getToolGroupSummaryFromParsedTools(tools: NodusSayTool[]): string {
	...
	return parts.length === 0 ? "Context" : "Nodus" + action + parts.join(", ")
}
