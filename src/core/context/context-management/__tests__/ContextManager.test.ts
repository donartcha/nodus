...
describe("ContextManager", () => {
	...
	describe("shouldCompactContextWindow", () => {
		...
		it("does not compact at 33K tokens with default 0.75 threshold on 200K context", () => {
			...
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn: 30_000, tokensOut: 3_000 })]

			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, 0, 0.75)
			expect(result).to.equal(false)
		})

		it("compacts when tokens exceed 0.75 threshold on 200K context", () => {
			const api = createMockApi(200_000)
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn: 140_000, tokensOut: 15_000 })]
		...
		it("compacts at only 10K tokens when threshold is accidentally set to 0.05", () => {
			...
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn, tokensOut })]

			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, 0, accidentalThreshold)
			expect(result).to.equal(true)
		})
		...
		it("falls back to maxAllowedSize when threshold is undefined", () => {
			...
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn: 150_000, tokensOut: 5_000 })]

			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, 0, undefined)
		...
		it("falls back to maxAllowedSize when threshold is 0", () => {
			...
			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, 0, 0)
			expect(result).to.equal(false)
		})
		...
		it("returns false when previousApiReqIndex is negative", () => {
			const api = createMockApi(200_000)
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn: 200_000 })]

			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, -1, 0.75)
			expect(result).to.equal(false)
		})
		...
		it("threshold is capped at maxAllowedSize even when percentage is very high", () => {
			...
			const nodusMessages: NodusMessage[] = [createApiReqMessage({ tokensIn: 165_000 })]

			const result = contextManager.shouldCompactContextWindow(nodusMessages, api, 0, 1.0)
			expect(result).to.equal(true)
		})
	})
})
