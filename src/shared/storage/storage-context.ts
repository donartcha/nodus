...
import { NodusFileStorage } from "./NodusFileStorage"
import { NodusMemento } from "./NodusStorage"
...
export interface StorageContext {
	...
	readonly globalState: NodusMemento
	 ...
	 *
	 * This split exists because CLI needs to intercept the NodusMemento interface to global state,
	 * but state resets need to write through to the backing store.
	 */
	readonly globalStateBackingStore: NodusFileStorage

	/** Secrets — API keys and other sensitive values. File uses restricted permissions (0o600). */
	readonly secrets: NodusFileStorage<string>
	/** Workspace-scoped state — per-project toggles, rules, etc. */
	readonly workspaceState: NodusFileStorage
	/** The resolved path to the data directory (~/.nodus/data) */
...
export interface StorageContextOptions {
	/**
	 * Override the Nodus home directory. Defaults to NODUS_DIR env var or ~/.nodus.
	 */
	NodusDir?: string
 ...
 *
 * File layout:
 *   ~/.nodus/data/globalState.json    — global state
 *   ~/.nodus/data/secrets.json        — secrets (mode 0o600)
 *   ~/.nodus/data/workspaces/<hash>/workspaceState.json — per-workspace state
 *
...
export function createStorageContext(opts: StorageContextOptions = {}): StorageContext {
	const NodusDir = opts.NodusDir || process.env.NODUS_DIR || path.join(os.homedir(), ".nodus")
	const dataDir = path.join(NodusDir, SETTINGS_SUBFOLDER)
	...
	const globalState = new NodusFileStorage(path.join(dataDir, "globalState.json"), "GlobalState")

	return {
		globalState,
		globalStateBackingStore: globalState,
		secrets: new NodusFileStorage<string>(path.join(dataDir, "secrets.json"), "Secrets", {
			fileMode: 0o600, // Owner read/write only — protects API keys
		}),
		workspaceState: new NodusFileStorage(path.join(workspaceDir, "workspaceState.json"), "WorkspaceState"),
		dataDir,
