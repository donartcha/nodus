...
import { NodusEnv, EnvironmentConfig } from "@/config"
...
interface NodusAuthApiUser {
	subject: string | null
	email: string
	name: string
	NodusUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface NodusAuthResponseData {
	/**
...
export interface NodusAuthApiTokenExchangeResponse {
	success: boolean
	data: NodusAuthResponseData
}

export interface NodusAuthApiTokenRefreshResponse {
	success: boolean
	data: NodusAuthResponseData
}

export class NodusAuthProvider {
	readonly name = "Nodus"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: NodusAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("nodus:nodusAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: NodusAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Nodus auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<NodusAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveNodusAuthInfo(controller: Controller): Promise<NodusAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("nodus:nodusAccountId")
			...
			let storedAuthData: NodusAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						// controller.stateManager.setSecret("NodusAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("nodus:nodusAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Nodus, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<NodusAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: NodusAuthInfo): Promise<NodusAuthInfo> {
		try {
			...
			const data: NodusAuthApiTokenExchangeResponse = await response.json()
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<NodusAuthInfo | null> {
		try {
			...
			const responseType: NodusAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("nodus:nodusAccountId", JSON.stringify(NodusAuthInfo))

			return NodusAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: NodusAuthApiTokenExchangeResponse["data"]): Promise<NodusAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${NodusEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.NodusUserId || "",
