...
import { ExplainChangesRequest } from "@shared/proto/nodus/task"
...
export async function explainChanges(controller: Controller, request: ExplainChangesRequest): Promise<Empty> {
	...
	try {
		...
		const messageIndex = nodusMessages.findIndex((m: any) => m.ts === request.messageTs)
		...
		const lastTaskCompletedMessageCheckpointHash = findLast(
			nodusMessages.slice(0, messageIndex),
			(m: any) => m.say === "completion_result",
		)?.lastCheckpointHash

		const firstCheckpointMessageCheckpointHash = nodusMessages.find(
