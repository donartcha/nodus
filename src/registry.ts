...
const prefix = name === "claude-dev" ? "Nodus" : name
 ...
 * In production, all commands are registered under "Nodus" for consistency.
 */
const NodusCommands = {
	...
	FixWithNodus: prefix + ".fixWithNodus",
...
export interface HostInfo {
	...
	os: string
	/**
	 * The type of the Nodus host environment, e.g. 'VSCode Extension', 'Nodus for JetBrains', 'CLI'
	...
	hostVersion?: string
	/**
	 * The version of Nodus that the host client is running
	 */
...
export const HostRegistryInfo = {
	init: async (distinctId: string) => {
		...
		const extensionVersion = host.nodusVersion || ExtensionRegistryInfo.version
		const platform = host.platform || "unknown"
		const os = process.platform || "unknown"
		const ide = host.nodusType || "unknown"
