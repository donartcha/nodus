...
/**
 * All valid hook types that can be created and executed by Nodus.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Nodus", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".nodusrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".nodusrules", "hooks")
}
