...
interface MessagesAreaProps {
	task: NodusMessage
	groupedMessages: (NodusMessage | NodusMessage[])[]
	modifiedMessages: NodusMessage[]
...
export const MessagesArea: React.FC<MessagesAreaProps> = ({
	...
	const { nodusMessages } = useExtensionState()
	const lastRawMessage = useMemo(() => nodusMessages.at(-1), [nodusMessages])
	...
	const scrolledPastUserMessageIndex = useMemo(() => {
		...
		return nodusMessages.findIndex((msg) => msg.ts === scrolledPastUserMessage.ts)
	}, [nodusMessages, scrolledPastUserMessage])
	...
	const displayedGroupedMessages = useMemo<(NodusMessage | NodusMessage[])[]>(() => {
		if (!showThinkingLoaderRow) {
			return groupedMessages
		}
		const waitingRow: NodusMessage = {
