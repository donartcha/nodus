...
import kanbanDemoVideoMp4 from "@/assets/Nodus_kanban_demo.mp4"
import kanbanDemoVideoWebm from "@/assets/Nodus_kanban_demo.webm"
import { Dialog, DialogContent } from "@/components/ui/dialog"
import { PLATFORM_CONFIG, PlatformType } from "@/config/platform.config"
import { FileServiceClient, StateServiceClient } from "@/services/grpc-client"

const INSTALL_COMMAND = "npm install -g nodus"
...
export const NODUS_KANBAN_MODAL_DISMISS_ID = "nodus-kanban-launch-modal-v1"

interface NodusKanbanLaunchModalProps {
	open: boolean
	onClose: (doNotShowAgain: boolean) => void
}

export const NodusKanbanLaunchModal: React.FC<NodusKanbanLaunchModalProps> = ({ open, onClose }) => {
	...
	const handleAction = async () => {
		if (isVsCode) {
			...
			try {
				await StateServiceClient.installNodusCli({})
			} catch (error) {
	...
	return (
		<Dialog onOpenChange={(isOpen) => !isOpen && onClose(doNotShowAgain)} open={open}>
			<DialogContent
				aria-describedby="Nodus-kanban-description"
				aria-labelledby="Nodus-kanban-title"
				className="pt-4 px-5 pb-4 gap-0 max-w-2xl">
				<div className="space-y-3" id="Nodus-kanban-description">
					<div className="pr-6 min-h-6 flex items-center">
						<h2
							className="m-0 text-lg font-semibold"
							id="Nodus-kanban-title"
							style={{ color: "var(--vscode-editor-foreground)" }}>
							Introducing Nodus Kanban
						</h2>
					</div>
...
export default NodusKanbanLaunchModal
