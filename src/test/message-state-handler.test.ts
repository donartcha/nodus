 ...
 * to prevent race conditions, particularly the TOCTOU bug in addTonodusMessages
 */
describe("MessageStateHandler Mutex Protection", () => {
	...
	/**
	 * Helper to create a test NodusMessage
	 */
	function createTestMessage(text: string): NodusMessage {
		return {
			ts: Date.now(),
			type: "say",
			say: "text",
	...
	it("should initialize with empty message arrays", () => {
		...
		handler.getnodusMessages().length.should.equal(0)
	...
	it("should set and get Nodus messages", () => {
		const handler = createTestHandler()
		const testMessages = [createTestMessage("test1"), createTestMessage("test2")]

		handler.setnodusMessages(testMessages)
		handler.getnodusMessages().should.deepEqual(testMessages)
	})

	/**
	 * CRITICAL TEST: Verify that addTonodusMessages is atomic
	 * This test simulates the race condition that can occur when multiple
	 * addTonodusMessages calls happen concurrently without proper mutex protection
	 */
	it("should handle concurrent addTonodusMessages atomically", async function () {
		...
		await handler.addTonodusMessages(initialMsg)

		// Verify initial state
		const messages = handler.getnodusMessages()
		...
		const results = await Promise.all([
			handler.addTonodusMessages(msg1),
			handler.addTonodusMessages(msg2),
			handler.addTonodusMessages(msg3),
		])
		...
		// Get final state
		const finalMessages = handler.getnodusMessages()
	...
	/**
	 * Test that updateNodusMessage operations are atomic
	 */
	it("should handle concurrent updateNodusMessage atomically", async function () {
		this.timeout(5000)
		...
		handler.setnodusMessages(msgs)

		// Perform concurrent updates to different messages
		await Promise.all([
			handler.updateNodusMessage(0, { text: "updated1" }),
			handler.updateNodusMessage(1, { text: "updated2" }),
			handler.updateNodusMessage(2, { text: "updated3" }),
		])
	...
	/**
	 * Test that deleteNodusMessage operations are atomic
	 */
	it("should handle deleteNodusMessage with proper validation", async () => {
		const handler = createTestHandler()
		...
		await handler.deleteNodusMessage(1)
	 ...
	 */
	it("should throw error for invalid message index in updateNodusMessage", async () => {
		const handler = createTestHandler()
		handler.setnodusMessages([createTestMessage("msg1")])

		try {
			await handler.updateNodusMessage(5, { text: "invalid" })
	...
	/**
	 * Test that invalid indices are rejected in deleteNodusMessage
	 */
	it("should throw error for invalid message index in deleteNodusMessage", async () => {
		const handler = createTestHandler()
		handler.setnodusMessages([createTestMessage("msg1")])
	 ...
	 */
	it("should handle overwritenodusMessages atomically", async () => {
		const handler = createTestHandler()

		// Set initial messages
		handler.setnodusMessages([createTestMessage("old1"), createTestMessage("old2")])

		// Overwrite with new messages
		const newMessages = [createTestMessage("new1"), createTestMessage("new2"), createTestMessage("new3")]
		await handler.overwritenodusMessages(newMessages)
