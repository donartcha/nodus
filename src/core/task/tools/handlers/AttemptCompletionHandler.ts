...
export class AttemptCompletionHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = NodusDefaultTool.ATTEMPT
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const addNewChangesFlagToLastCompletionResultMessage = async () => {
			...
			const nodusMessages = config.messageState.getnodusMessages()

			const lastCompletionResultMessageIndex = findLastIndex(nodusMessages, (m: any) => m.say === "completion_result")
			const lastCompletionResultMessage =
				lastCompletionResultMessageIndex !== -1 ? nodusMessages[lastCompletionResultMessageIndex] : undefined
			if (
				...
				await config.messageState.updateNodusMessage(lastCompletionResultMessageIndex, {
		...
		if (partialCompletionIndex !== -1) {
			const updatedMessages = [
				...nodusMessages.slice(0, partialCompletionIndex),
				...nodusMessages.slice(partialCompletionIndex + 1),
			]
			config.messageState.setnodusMessages(updatedMessages)
			await config.messageState.savenodusMessagesAndUpdateHistory()
		}

		let commandResult: any
		const lastMessage = config.messageState.getnodusMessages().at(-1)
		...
		if (command) {
			...
			const autoApproveResult = config.autoApprover?.shouldAutoApproveTool(NodusDefaultTool.BASH)
			...
			if (autoApproveSafe) {
				...
				showNotificationForApproval(
					`Nodus wants to execute a command: ${command}`,
		...
		if (config.messageState.getnodusMessages().at(-1)?.ask === "command_output") {
