...
import { WorktreeDefaults } from "@shared/proto/nodus/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/Nodus-${suffix}`

	// Generate suggested path in Documents/Nodus/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Nodus", "Worktrees", `${projectName}-${suffix}`)
