...
import type { NodusTool } from "@/shared/tools"
...
export class AwsBedrockHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: NodusStorageMessage[], tools?: NodusTool[]): ApiStream {
	...
	private async getAwsCredentials(): Promise<{
		...
		const providerOptions: ProviderChainOptions = {
			clientConfig: {
				...
				userAgentAppId: `Nodus#${ExtensionRegistryInfo.version}`,
			},
		}
		const useProfile =
	 ...
	 */
	private formatDeepseekR1Prompt(systemPrompt: string, messages: NodusStorageMessage[]): string {
	 ...
	 */
	private estimateInputTokens(systemPrompt: string, messages: NodusStorageMessage[]): number {
	...
	/**
	 * Converts Nodus's tool definitions (Anthropic format with `input_schema`) to the
	 ...
	 */
	private mapNodusToolsToBedrockToolConfig(tools?: NodusTool[]): ToolConfiguration | undefined {
		if (!tools || tools.length === 0) {
			return undefined
		}

		const isAnthropicTool = (tool: NodusTool): tool is AnthropicTool => "input_schema" in tool
	...
	private async *createAnthropicMessage(
		...
		const toolConfig = this.mapNodusToolsToBedrockToolConfig(tools)
	 ...
	 */
	private formatMessagesForConverseAPI(messages: NodusStorageMessage[]): Message[] {
