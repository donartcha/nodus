...
/**
 * The raw model information returned by the Nodus API to list models
 */
interface NodusRawModelInfo {
	id: string
	name: string
	...
	tiers?: any[] | null
	supported_parameters?: NodusSupportedParams[] | null
}
...
async function fetchRawnodusModels(): Promise<NodusRawModelInfo[]> {
	const apiBaseUrl = NodusEnv.config().apiBaseUrl
	const response = await axios.get(`${apiBaseUrl}/api/v1/ai/Nodus/models`, getAxiosSettings())

	if (!Array.isArray(response.data?.data)) {
		throw new Error("Invalid response data when fetching Nodus models")
	}

	Logger.log("Nodus models source: Nodus API")
	return response.data.data as NodusRawModelInfo[]
}

/**
 * Core function: Refreshes the Nodus models and returns application types
...
export async function refreshNodusModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	const cache = StateManager.get().getModelsCache("Nodus")
	if (cache) {
		return cache
	}
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCachenodusModels()
		} finally {
...
async function fetchAndCachenodusModels(): Promise<Record<string, ModelInfo>> {
	const nodusModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.nodusModels)

	let models: Record<string, ModelInfo> = {}
	try {
		const rawModels = await fetchRawnodusModels()
		...
		if (Object.keys(models).length === 0) {
			throw new Error("No Nodus models returned from API")
		}
		// Save models and cache them in memory
		await fs.writeFile(nodusModelsFilePath, JSON.stringify(models))
		Logger.log("Nodus models fetched and saved")
	} catch (error) {
		Logger.error("Error fetching Nodus models:", error)

		// If we failed to fetch models, try to read cached models from disk
		try {
			const fileExists = await fileExistsAtPath(nodusModelsFilePath)
			if (fileExists) {
				const fileContents = await fs.readFile(nodusModelsFilePath, "utf8")
				models = JSON.parse(fileContents)
				Logger.log("Loaded Nodus models from cache")
			}
		} catch (cacheError) {
			Logger.error("Error reading Nodus models from cache:", cacheError)
		}
	}

	// Avoid poisoning in-memory cache with an empty model map after transient failures.
	if (Object.keys(models).length > 0) {
		StateManager.get().setModelsCache("Nodus", models)
	}
...
/**
 * Read cached Nodus models from disk
 * @returns The cached models or undefined if not found
 */
export async function readnodusModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		const nodusModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.nodusModels)
		const fileExists = await fileExistsAtPath(nodusModelsFilePath)
		if (fileExists) {
		...
		Logger.error("Error reading Nodus models from cache:", error)
	}
	return undefined
}
