...
import { BrowserAction, BrowserActionResult, NodusMessage, NodusSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: NodusMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: NodusMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: NodusMessage[] = []
		let nextActionMessages: NodusMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Nodus is using the browser:" : "Nodus wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
