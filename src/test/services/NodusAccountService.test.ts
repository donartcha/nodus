...
describe("NodusAccountService.fetchUserRemoteConfig", () => {
	let service: NodusAccountService
	...
	beforeEach(() => {
		...
		service = new NodusAccountService()
	})
...
describe("NodusAccountService.fetchFeaturebaseToken", () => {
	let service: NodusAccountService
	let sandbox: sinon.SinonSandbox
	...
	it("returns undefined when the request throws due to missing auth token", async () => {
		sandbox
			.stub(service as unknown as { authenticatedRequest: () => unknown }, "authenticatedRequest")
			.rejects(new Error("No Nodus account auth token found"))
