...
import { NodusConfigurationError, NodusEndpoint, NodusEnv, Environment } from "../config"

describe("NodusEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		;(NodusEndpoint as any)._instance = null
		;(NodusEndpoint as any)._initialized = false
		;(NodusEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			const config = NodusEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.nodus.bot")
			config.apiBaseUrl.should.equal("https://api.nodus.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				appBaseUrl: "https://proxy.enterprise.com/Nodus/app",
				apiBaseUrl: "https://proxy.enterprise.com/Nodus/api",
			...
			const config = NodusEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/Nodus/app")
		})
	})
	...
	describe("invalid JSON handling", () => {
		it("should throw NodusConfigurationError for invalid JSON syntax", async () => {
			...
			try {
				...
				error.should.be.instanceof(NodusConfigurationError)
	...
	describe("missing required fields", () => {
		it("should throw NodusConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw NodusConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw NodusConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw NodusConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw NodusConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw NodusConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw NodusConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw NodusConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw NodusConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw NodusConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			NodusEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				NodusEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should allow environment switching in standard mode", async () => {
			...
			NodusEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			NodusEnv.setEnvironment("staging")
			NodusEnv.getEnvironment().environment.should.equal("staging")

			NodusEnv.setEnvironment("local")
			NodusEnv.getEnvironment().environment.should.equal("local")

			NodusEnv.setEnvironment("production")
			NodusEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = NodusEndpoint.config
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await NodusEndpoint.initialize(tempDir)
			NodusEndpoint.isInitialized().should.be.true()
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			NodusEndpoint.isInitialized().should.be.false()
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			NodusEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should throw NodusConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw NodusConfigurationError for invalid JSON in bundled file", async () => {
