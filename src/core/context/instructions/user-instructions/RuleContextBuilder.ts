...
type MessageStateHandlerLike = {
	getnodusMessages(): NodusMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Nodus Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in nodusMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const nodusMessages = deps.messageStateHandler.getnodusMessages()
		...
		const lastUserMsg = [...nodusMessages]
			.reverse()
		...
		// (3) Files edited by Nodus during this task (completed operations):
