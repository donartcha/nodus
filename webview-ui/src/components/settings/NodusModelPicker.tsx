...
export interface NodusModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
function toFeaturedModelCardEntry(
	model: Pick<NodusRecommendedModel, "id" | "description" | "tags">,
...
const NodusModelPicker: React.FC<NodusModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	...
	const [NodusRecommendedModels, setNodusRecommendedModels] = useState<FeaturedModelCardEntry[]>([])
	const [NodusFreeModels, setNodusFreeModels] = useState<FeaturedModelCardEntry[]>([])
	const freeNodusModelIds = useMemo(() => {
		const freeModelIds =
			NodusFreeModels.length > 0 ? NodusFreeModels.map((model) => model.id) : FREE_MODELS_FALLBACK.map((model) => model.id)
		return [...new Set(freeModelIds)]
	}, [NodusFreeModels])
	const freeNodusModelIdSet = useMemo(
		() => new Set(freeNodusModelIds.map((modelId) => normalizeModelId(modelId))),
		[freeNodusModelIds],
	)
	const [activeTab, setActiveTab] = useState<"recommended" | "free">(initialTab ?? "recommended")
	const recommendedModels = useMemo(
		() => (NodusRecommendedModels.length > 0 ? NodusRecommendedModels : RECOMMENDED_MODELS_FALLBACK),
		[NodusRecommendedModels],
	)
	const freeModels = useMemo(() => (NodusFreeModels.length > 0 ? NodusFreeModels : FREE_MODELS_FALLBACK), [NodusFreeModels])
	const hasSuccessfulNodusRecommendedModelsFetchRef = useRef(false)
	const isFetchingNodusRecommendedModelsRef = useRef(false)
	const NodusRecommendedModelsRetryTimeoutRef = useRef<number | null>(null)

	const refreshNodusRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			const response = await ModelsServiceClient.makeUnaryRequest(
				"refreshNodusRecommendedModelsRpc",
				EmptyRequest.create({}),
				EmptyRequest.toJSON,
				NodusRecommendedModelsResponse.fromJSON,
			)
			...
			const free = (response.free ?? [])
				.map((model) => toFeaturedModelCardEntry(model, "FREE"))
				.filter((model): model is FeaturedModelCardEntry => model !== null)
			setNodusRecommendedModels(recommended)
			setNodusFreeModels(free)
			return true
		} catch (error) {
			console.error("Failed to refresh Nodus recommended models:", error)
			return false
		}
	}, [])

	const clearNodusRecommendedModelsRetryTimeout = useCallback(() => {
		if (NodusRecommendedModelsRetryTimeoutRef.current !== null) {
			window.clearTimeout(NodusRecommendedModelsRetryTimeoutRef.current)
			NodusRecommendedModelsRetryTimeoutRef.current = null
		}
	}, [])

	const fetchNodusRecommendedModels = useCallback(async () => {
		if (hasSuccessfulNodusRecommendedModelsFetchRef.current || isFetchingNodusRecommendedModelsRef.current) {
			return
		}
		isFetchingNodusRecommendedModelsRef.current = true
		const succeeded = await refreshNodusRecommendedModels()
		isFetchingNodusRecommendedModelsRef.current = false

		if (succeeded) {
			hasSuccessfulNodusRecommendedModelsFetchRef.current = true
			clearNodusRecommendedModelsRetryTimeout()
			return
		}
	...
	useEffect(() => {
		...
		const currentModelId = modeFields.NodusModelId || openRouterDefaultModelId
		setActiveTab(freeNodusModelIdSet.has(normalizeModelId(currentModelId)) ? "free" : "recommended")
	}, [modeFields.NodusModelId, freeNodusModelIdSet, initialTab])
	...
	const handleModelChange = (newModelId: string) => {
		...
		handleModeFieldsChange(
			{
				NodusModelId: { plan: "planModeNodusModelId", act: "actModeNodusModelId" },
				NodusModelInfo: { plan: "planModeNodusModelInfo", act: "actModeNodusModelInfo" },
			},
			{
				NodusModelId: newModelId,
				NodusModelInfo: nodusModels?.[newModelId],
			},
			currentMode,
		)
	}
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		if (freeNodusModelIdSet.has(normalizeModelId(selected.selectedModelId))) {
			return {
				...selected,
				selectedModelInfo: {
	...
	useEffect(() => {
		void fetchNodusRecommendedModels()
	}, [fetchNodusRecommendedModels])

	// Sync external changes when the modelId changes
	useEffect(() => {
		const currentModelId = modeFields.NodusModelId || openRouterDefaultModelId
		setSearchTerm(currentModelId)
	}, [modeFields.NodusModelId])
	...
	const modelIds = useMemo(() => {
		const unfilteredModelIds = Object.keys(nodusModels ?? {}).sort((a, b) => a.localeCompare(b))
		return filterOpenRouterModelIds(unfilteredModelIds, "Nodus", freeNodusModelIds)
	}, [nodusModels, freeNodusModelIds])
	...
	const showBudgetSlider = useMemo(() => {
		...
		return (
			Object.entries(nodusModels ?? {})?.some(([id, m]) => id === selectedModelId && m.thinkingConfig) ||
	...
	}, [nodusModels, selectedModelId, selectedModelIdLower, showReasoningEffort])
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				...
				<p
					...
					The extension automatically fetches the latest Nodus model list. If you're unsure which model to choose, Nodus
...
export default NodusModelPicker
