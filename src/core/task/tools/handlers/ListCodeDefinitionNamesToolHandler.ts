...
export class ListCodeDefinitionNamesToolHandler implements IFullyManagedTool {
	readonly name = NodusDefaultTool.LIST_CODE_DEF
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			result = await parseSourceCodeForDefinitionsTopLevel(absolutePath, config.services.NodusIgnoreController)
		} catch (error) {
			config.taskState.consecutiveMistakeCount++
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Nodus wants to analyze code definitions in ${getWorkspaceBasename(absolutePath, "ListCodeDefinitionNamesToolHandler.notification")}`
