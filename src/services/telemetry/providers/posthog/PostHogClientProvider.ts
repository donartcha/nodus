...
import { NodusEndpoint } from "@/config"
...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Nodus extension.
	 * this is specifically to avoid capturing errors from anything other than Nodus
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Nodus
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "Nodus"
			if (stacktrace?.value?.toLowerCase().includes("Nodus")) {
				return event
			}
			...
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					...
					// The CLI filename will include "Nodus"
					if (fileName?.includes("saoudrizwan") || fileName?.includes("Nodus")) {
						return event
					}
				}
			}
