import { NodusMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the NodusSayHook interface but represents parsed data.
 */
...
function getSay(msg: NodusMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: NodusMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: NodusMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: NodusMessage[]): NodusMessage[] {
...
function combineHookWithOutputs(
	hookMessage: NodusMessage,
	startIndex: number,
	messages: NodusMessage[],
): { combined: NodusMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: NodusMessage[]): NodusMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, NodusMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: NodusMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: NodusMessage[], originalMessages: NodusMessage[]): Map<number, NodusMessage[]> {
	const map = new Map<number, NodusMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: NodusMessage[], preToolUseMap: Map<number, NodusMessage[]>): NodusMessage[] {
	const result: NodusMessage[] = []
 ...
 *
 * @param messages Array of NodusMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: NodusMessage[]): NodusMessage[] {
