...
export class NodusError extends Error {
	...
	/**
	 * Parses a stringified error into a NodusError instance.
	 */
	static parse(errorStr?: string, modelId?: string): NodusError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return NodusError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a NodusError instance.
	 * Always returns a NodusError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): NodusError {
		try {
			// If already a NodusError, return it directly to prevent infinite recursion
			if (error instanceof NodusError) {
				return error
			}
			return new NodusError(JSON.parse(error), modelId, providerId)
		} catch {
			return new NodusError(error, modelId, providerId)
		}
	}

	public isErrorType(type: NodusErrorType): boolean {
		return NodusError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: NodusError): NodusErrorType | undefined {
		...
		if (code === "SPEND_LIMIT_EXCEEDED" || details?.code === "SPEND_LIMIT_EXCEEDED") {
			return NodusErrorType.SpendLimit
		}
		...
		if (code === "INFERENCE_CAP_ERROR") {
			return NodusErrorType.QuotaExceeded
		}
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return NodusErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = NodusErrorType.Network
	}
}
