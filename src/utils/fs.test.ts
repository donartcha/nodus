...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "Nodus-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .nodusrules/workflows directory specifically", async () => {
		...
		const allFiles = await readDirectory(NodusrulesDirPath)
		...
		const expectedFiles = [
			path.resolve(NodusrulesDirPath, "config.json"),
			path.resolve(NodusrulesDirPath, "settings.js"),
		...
		const rootOnlyFiles = [path.resolve(NodusrulesDirPath, "config.json"), path.resolve(NodusrulesDirPath, "settings.js")]
