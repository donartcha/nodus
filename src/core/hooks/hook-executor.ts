...
import type { HookOutput } from "@shared/proto/nodus/hooks"
...
async function updateHookMessage(
	...
	const nodusMessages = messageStateHandler.getnodusMessages()
	const hookMessageIndex = nodusMessages.findIndex((m: NodusMessage) => m.ts === hookMessageTs)
	if (hookMessageIndex !== -1) {
		await messageStateHandler.updateNodusMessage(hookMessageIndex, {
...
async function reorderHookAndToolMessages(messageStateHandler: MessageStateHandler): Promise<void> {
	...
	for (let i = nodusMessages.length - 1; i >= 0; i--) {
		const msgType = nodusMessages[i].ask || nodusMessages[i].say
	...
	for (let i = lastToolMessageIndex + 1; i < nodusMessages.length; i++) {
		if (nodusMessages[i].say === "hook_status" || nodusMessages[i].say === "hook_output_stream") {
	...
	const toolMessage = { ...nodusMessages[lastToolMessageIndex] }

	// Delete the tool message at its current position
	await messageStateHandler.deleteNodusMessage(lastToolMessageIndex)

	// Re-add the tool message at the end (after hook messages)
	await messageStateHandler.addTonodusMessages(toolMessage)
}
