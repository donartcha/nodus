...
import { NodusAuthContext, NodusAuthContextType, NodusAuthProvider, useNodusAuth } from "@/context/NodusAuthContext"
import {
...
const NodusAuthProviderWithOverrides: React.FC<{
	overrides?: Partial<NodusAuthContextType>
	children: React.ReactNode
}> = ({ overrides, children }) => {
	const authContext = useNodusAuth()
	return <NodusAuthContext.Provider value={{ ...authContext, ...overrides }}>{children}</NodusAuthContext.Provider>
}

export const createStorybookDecorator =
	(overrideStates?: Partial<ExtensionStateContextType>, classNames?: string, authOverrides?: Partial<NodusAuthContextType>) =>
	(Story: any) => (
		<ExtensionStateProviderWithOverrides overrides={overrideStates}>
			<NodusAuthProviderWithOverrides overrides={authOverrides}>
				<div className={cn("max-w-lg mx-auto", classNames)}>
					<Story />
				</div>
			</NodusAuthProviderWithOverrides>
