...
describe("listFiles", () => {
	const tmpDir = path.join(os.tmpdir(), `Nodus-list-files-test-${Math.random().toString(36).slice(2)}`)
...
describe("listFiles gitignore handling", () => {
	...
	const baseDir = path.join(os.tmpdir(), `Nodus-gitignore-test-${Math.random().toString(36).slice(2)}`)
