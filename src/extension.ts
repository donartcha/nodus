...
import { addToNodus } from "./core/controller/commands/addToNodus"
import { explainWithNodus } from "./core/controller/commands/explainWithNodus"
import { fixWithNodus } from "./core/controller/commands/fixWithNodus"
import { improveWithNodus } from "./core/controller/commands/improveWithNodus"
...
import { registerNodusOutputChannel } from "./hosts/vscode/hostbridge/env/debugLog"
import {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// After this, all platforms (VSCode, CLI, JetBrains) read from ~/.nodus/data/.
	...
	*/
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		if (isTaskUri) {
			await openNodusSidebarForTaskUri()
		}
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "nodus.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[Nodus Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log("[Nodus Dev] Failed to register dev commands: " + error)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Nodus (Always available)
					const addAction = new vscode.CodeAction("Add to Nodus", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					// Explain with Nodus (Always available)
					const explainAction = new vscode.CodeAction("Explain with Nodus", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with Nodus",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Nodus (Always available)
					const improveAction = new vscode.CodeAction("Improve with Nodus", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with Nodus",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Nodus (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Nodus", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithNodus,
							title: "Fix with Nodus",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.AddToChat, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			...
			await addToNodus(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.FixWithNodus, async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
			await fixWithNodus(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ExplainCode, async (range: vscode.Range) => {
			...
			await explainWithNodus(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ImproveCode, async (range: vscode.Range) => {
			...
			await improveWithNodus(context.controller, context.commandContext)
		}),
	)
...
\`\`\``
				...
				await addToNodus(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				...
				await explainWithNodus(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
...
\`\`\``
				...
				await improveWithNodus(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#NodusWalkthrough`)
	...
	const unsubSecrets = storageContext.secrets.onDidChange((event) => {
		if (event.key === "nodus:nodusAccountId") {
	...
	Logger.log(`[Nodus] extension activated in ${performance.now() - activationStartTime} ms`)

	return createNodusAPI(webview.controller)
}
...
function setupHostProvider(context: ExtensionContext) {
	const outputChannel = registerNodusOutputChannel(context)
	outputChannel.appendLine("[Nodus] Setting up VS Code host...")
...
async function openNodusSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for Nodus sidebar visibility")
}
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global Nodus rules (one-time cleanup)
