...
import { refreshNodusRecommendedModels } from "@/core/controller/models/refreshNodusRecommendedModels"
...
interface NodusHandlerOptions extends CommonApiHandlerOptions {
	ulid?: string
	taskId?: string
...
export class NodusHandler implements ApiHandler {
	private options: NodusHandlerOptions
	private NodusAccountService = NodusAccountService.getInstance()
	...
	private get _baseUrl(): string {
		return NodusEnv.config().apiBaseUrl
	}

	constructor(options: NodusHandlerOptions) {
	...
	private async ensureClient(): Promise<OpenAI> {
		...
		if (!this.client) {
			try {
				...
				Object.assign(defaultHeaders, await buildNodusExtraHeaders())

				this.client = new OpenAI({
					baseURL: `${this._baseUrl}/api/v1`,
					apiKey: NodusAccountAuthToken,
					defaultHeaders,
				...
				throw new Error(`Error creating Nodus client: ${error.message}`)
			}
		}
		// Ensure the client is always using the latest auth token
		this.client.apiKey = NodusAccountAuthToken
		return this.client
	}

	@withRetry()
	async *createMessage(systemPrompt: string, messages: NodusStorageMessage[], tools?: OpenAITool[]): ApiStream {
		try {
			...
			for await (const chunk of stream) {
				Logger.debug("NodusHandler chunk:" + JSON.stringify(chunk))
				// openrouter returns an error object instead of the openai sdk throwing an error
				if ("error" in chunk) {
					const error = chunk.error as OpenRouterErrorResponse["error"]
					Logger.error(`Nodus API Error: ${error?.code} - ${error?.message}`)
					// Include metadata in the error message if available
					const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
					throw new Error(`Nodus API Error ${error.code}: ${error.message}${metadataStr}`)
				}
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						Logger.error(`Nodus Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
						throw new Error(`Nodus Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
					}
					throw new Error("Nodus Mid-Stream Error: Stream terminated with error status but no error details provided")
				}
			...
			if (!didOutputUsage) {
				Logger.warn("Nodus API did not return usage chunk, fetching from generation endpoint")
			...
			Logger.error("Nodus API Error:", error)
			throw error
		}
	}
		...
		if (this.lastGenerationId) {
			try {
				...
				const headers: Record<string, string> = {
					...
					Authorization: `Bearer ${NodusAccountAuthToken}`,
				}
				Object.assign(headers, await buildNodusExtraHeaders())

				const response = await axios.get(`${this.NodusAccountService.baseUrl}/generation?id=${this.lastGenerationId}`, {
					headers,
				...
				// ignore if fails
				Logger.error("Error fetching Nodus generation details:", error)
			}
		}
		return undefined
	}
