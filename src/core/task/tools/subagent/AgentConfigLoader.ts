...
import { NodusDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/Nodus/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(NodusDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): NodusDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as NodusDefaultTool
	if (Object.values(NodusDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a NodusDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): NodusDefaultTool[] {
	if (!tools) {
		return []
	}
...
export function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "Nodus", AGENTS_CONFIG_DIRECTORY_NAME)
}
