...
const NodusRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setglobalNodusRulesToggles,
		setlocalNodusRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalNodusRulesToggles?.toggles) {
						setglobalNodusRulesToggles(response.globalNodusRulesToggles.toggles)
					}
					if (response.localNodusRulesToggles?.toggles) {
						setlocalNodusRulesToggles(response.localNodusRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalNodusRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localNodusRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleNodusRule(
			ToggleNodusRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Nodus rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: NodusRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Nodus Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Nodus Rules & Workflows" : "Show Nodus Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Nodus with system-level guidance. Think of them as a persistent way
								...
								<p>
									Workflows allow you to define a series of steps to guide Nodus through a repetitive set of
									...
									<VSCodeLink
										...
										href="https://docs.nodus.bot/features/slash-commands/workflows">
										Docs
									</VSCodeLink>
								</p>
							) : currentView === "skills" ? (
								<p>
									Skills are reusable instruction sets that Nodus can activate on-demand. When a task matches a
									skill's description, Nodus uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Nodus's execution lifecycle,
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										...
										<VSCodeLink
											className="text-xs"
											href="https://docs.nodus.bot/features/hooks"
											style={{ display: "inline", fontSize: "inherit" }}>
											Docs
										</VSCodeLink>
									</p>
...
export default NodusRulesToggleModal
