...
import { NodusApiReqInfo, NodusMessage, NodusSay } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { HistoryItem } from "@shared/HistoryItem"
import { NodusCheckpointRestore } from "@shared/WebviewMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const nodusMessages = this.services.messageStateHandler.getnodusMessages()
			nodusMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getnodusMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.savenodusMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								Logger.error(
				...
				const lastFivenodusMessages = this.services.messageStateHandler.getnodusMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFivenodusMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: NodusCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const nodusMessages = this.services.messageStateHandler.getnodusMessages()
			const messageIndex = nodusMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(nodusMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = nodusMessages[messageIndex]
			const lastMessageWithHash = nodusMessages[lastHashIndex]
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			const nodusMessages = this.services.messageStateHandler.getnodusMessages()
			const messageIndex = nodusMessages.findIndex((m) => m.ts === messageTs)
			const message = nodusMessages[messageIndex]
			if (!message) {
				Logger.error(`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`)
				relinquishButton()
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getnodusMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const nodusMessages = this.services.messageStateHandler.getnodusMessages()
			const messageIndex = findLastIndex(nodusMessages, (m) => m.say === "completion_result")
			const message = nodusMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const nodusMessages = this.services.messageStateHandler.getnodusMessages()
				const deletedMessages = nodusMessages.slice(messageIndex + 1)
				...
				const newnodusMessages = nodusMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwritenodusMessages(newnodusMessages) // calls savenodusMessages which saves historyItem
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Nodus in a project that uses git, or disabling checkpoints.",
					)
				}
			}, 7_000)
			...
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening Nodus in a project that uses git, or disabling checkpoints.",
				},
			)
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening Nodus in a project that uses git, or disabling checkpoints.",
				)
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
