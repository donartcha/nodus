...
import { NodusStorage } from "./NodusStorage"
import type { BlobStoreSettings } from "./types"

export type { BlobStoreSettings } from "./types"

/**
 * S3/R2/Azure blob storage implementation of NodusStorage.
 * Uses AWS S3, Cloudflare R2, or Azure Blob Storage as the backend storage.
 */
export class NodusBlobStorage extends NodusStorage {
	override name = "NodusBlobStorage"

	private static store: NodusBlobStorage | null = null
	static get instance(): NodusBlobStorage {
		if (!NodusBlobStorage.store) {
			NodusBlobStorage.store = new NodusBlobStorage()
		}
		return NodusBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!NodusBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[NodusBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[NodusBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = NodusBlobStorage.instance
