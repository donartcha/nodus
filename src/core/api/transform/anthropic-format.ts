...
import { NodusStorageMessage, convertNodusStorageToAnthropicMessage } from "@/shared/messages/content"

/**
 * Converts Nodus storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param nodusMessages - Array of Nodus storage messages to convert
...
export function sanitizeAnthropicMessages(
	nodusMessages: Array<NodusStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = nodusMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return nodusMessages.map((msg, index) => {
		const anthropicMsg = convertNodusStorageToAnthropicMessage(msg)
