import type { NodusAsk, NodusSay } from "@shared/ExtensionMessage"
import type { NodusDefaultTool } from "@shared/tools"
import type { NodusAskResponse } from "@shared/WebviewMessage"
...
export interface StronglyTypedUIHelpers {
	// Core UI methods
	say: (type: NodusSay, text?: string, images?: string[], files?: string[], partial?: boolean) => Promise<number | undefined>
	...
	removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: NodusAsk | NodusSay) => Promise<void>

	// Approval methods
	shouldAutoApproveTool: (toolName: NodusDefaultTool) => boolean | [boolean, boolean]
	shouldAutoApproveToolWithPath: (toolName: NodusDefaultTool, path?: string) => Promise<boolean>
	askApproval: (messageType: NodusAsk, message: string) => Promise<boolean>

	// Telemetry and notifications
	captureTelemetry: (toolName: NodusDefaultTool, autoApproved: boolean, approved: boolean, isNativeToolCall?: boolean) => void
...
export function createUIHelpers(config: TaskConfig): StronglyTypedUIHelpers {
	return {
		...
		shouldAutoApproveTool: (toolName: NodusDefaultTool) => config.autoApprover.shouldAutoApproveTool(toolName),
		shouldAutoApproveToolWithPath: config.callbacks.shouldAutoApproveToolWithPath,
		askApproval: async (messageType: NodusAsk, message: string): Promise<boolean> => {
			const { response } = await config.callbacks.ask(messageType, message, false)
			return response === "yesButtonClicked"
		},
		captureTelemetry: (toolName: NodusDefaultTool, autoApproved: boolean, approved: boolean, isNativeToolCall?: boolean) => {
