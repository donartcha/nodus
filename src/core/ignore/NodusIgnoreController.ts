...
export class NodusIgnoreController {
	...
	NodusIgnoreContent: string | undefined
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processNodusIgnoreIncludes(content)
	 ...
	 */
	private async processNodusIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[NodusIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
