/**
 * NodusTempManager - Manages temporary files for Nodus with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "Nodus" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Nodus's temporary files.
 */
class NodusTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "Nodus" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/Nodus
		// Windows: C:\Users\{user}\AppData\Local\Temp\Nodus
		// Linux: /tmp/Nodus
		const baseTempDir = os.tmpdir()
		const NodusTempDir = path.join(baseTempDir, "Nodus")

		try {
			fs.mkdirSync(NodusTempDir, { recursive: true })
			this.tempDir = NodusTempDir
		} catch {
	...
	/**
	 * Clean up old Nodus temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Nodus temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Nodus temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Nodus temp cleanup", error)
		}
...
export const NodusTempManager = new NodusTempManagerImpl()
