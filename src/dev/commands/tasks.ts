...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("Nodus.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("Nodus.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): NodusMessage[] {
	...
	const messages: NodusMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Nodus.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
