...
function parseSubagentRowData(message: NodusMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as NodusAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as NodusSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "Nodus wants to use a subagent:" : "Nodus wants to use subagents:"
