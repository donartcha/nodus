...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .Nodus/skills directory", async () => {
			...
			fileExistsStub.withArgs(NodusSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(NodusSkillsDir).resolves(true)
			readdirStub.withArgs(NodusSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
